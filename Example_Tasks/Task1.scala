...
object Task1 {
  ...
  def main(args: Array[String]) : Unit = { 
    ...
    //PART1 - OPTION
    val optionalUser12 = UsersDAO.findOption(1)
    if (optionalUser12.isDefined) {
          displayPage(userToHTML(optionalUser12.get))
    }
    else{
    ...
    case z: Exception =>displayError("There is no user with this id")
    }
    //}
